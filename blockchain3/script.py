...
class Script(object):
    ...
    def sign(privkey, data):
        ...
        return Dilithium2.sign(privkey, bytes(data))
    ...
    def verify(data, signature, pubkey):
        ...
        if data == None or signature == None or pubkey == None:
            return False
        # try:
        #     Dilithium2.verify(pubkey, data, signature)
        #     # rsa.verify(data.encode("utf-8"), signature, pubkey)
        # except:
        #     return False
        #     return True
        return Dilithium2.verify(pubkey, bytes(data.encode("utf-8")), signature)
...
def get_address_from_ripemd160(ripemd_hash):
    Wallet.b58encode(unhexlify(ripemd_hash.encode("utf-8").decode('utf8')))
    return Wallet.b58encode(unhexlify(ripemd_hash.encode("utf-8").decode('utf8')))
